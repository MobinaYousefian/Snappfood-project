                ...
                <div className={" slidersCategories-transform relative w-full h-full z-10 flex"}>
                    {
                        restaurants.map(({name, id, avatar, banner, category, star, rating, delivery, coupon, couponList, discountNumber, workHour }) => (
                            <div key={id} className={"1288:w-[24%] 947:w-[32.165%] sp-tablet:w-[48.8%] box-border transition-transform relative h-full shrink-0 ml-[16px] w-full"}>
                                <Link href={"/"}>
                                    <div className={"hover:shadow-sp-high text-carbon-main shadow-sp-small rounded-lg flex-col flex border-[1px] border-carbon-alphaLight border-solid bg-surface-light overflow-hidden pb-6 mb-8 box-border justify-between h-[21.3125rem]"}>
                                        <div className={"relative shrink-0 grow flex justify-center items-end min-h-[9.6875rem] max-h-[9.6875rem] h-[9.6875rem]"}>
                                            ...
                                            <div className={clsx( coupon ? "block" : "hidden" ,"items-center flex rounded-bl-2xl rounded-tl-2xl bg-surface-light pl-3 pr-2 py-1 absolute top-5 right-0 left-auto max-w-[calc(100% - 26px)]")}>
